...
export async function fetchPlaylist(): Promise<{ playlist: Playlist, tracks: Omit<Track, 'isLiked'>[] }> {
  const { data } = await apolloClient.query<gql_GetPlaylistQuery>({
    query: GetPlaylistDocument,
  })

  const tracks = data.playlist.tracks?.reduce<Omit<Track, 'isLiked'>[]>((acc, playlistTrack) => {
